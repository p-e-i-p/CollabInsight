...
test.describe('任务管理功能测试', () => {
  ...
  test('应该能够创建新任务', async ({ page }) => {
    ...
    // 点击添加任务按钮 - 按钮文本是"添加"，不是"添加任务"
    const addTaskButton = page.locator('button:has-text("添加")').first();
    await addTaskButton.waitFor({ state: 'visible', timeout: 10000 });
    
    // 检查按钮是否被禁用
    const isDisabled = await addTaskButton.isDisabled();
    if (isDisabled) {
      console.log('添加任务按钮被禁用，可能需要先选择项目');
      await projectItems.first().click();
      await page.waitForTimeout(2000);
    }
    ...
    // 填写任务信息 - 使用Modal内的输入框，排除搜索框
    const taskName = `测试任务_${Date.now()}`;
    const nameInput = page.locator('.ant-modal input[placeholder*="任务名称"]:not([placeholder*="搜索"])').first();
    ...
    // 选择分配人（必填）- 使用 Modal 内的 Select，强制点击
    ...
    // 填写任务详情（必填）
    const taskDetailsInput = page.locator('textarea[placeholder*="任务详情"]').first();
    if (await taskDetailsInput.isVisible({ timeout: 3000 })) {
      await taskDetailsInput.fill('这是测试任务详情');
    }
    ...
    // 选择紧急程度（必填）- 通过placeholder定位
    const urgencySelectInput = page.locator('input[placeholder*="紧急程度"]').first();
    const isUrgencyVisible = await urgencySelectInput.isVisible({ timeout: 3000 }).catch(() => false);
    if (isUrgencyVisible) {
      // 先关闭之前可能打开的下拉菜单
      await page.keyboard.press('Escape');
      await page.waitForTimeout(300);
      
      await urgencySelectInput.click({ force: true });
      await page.waitForTimeout(500);
      
      // 等待新的下拉菜单出现
      const urgencyOption = page.locator('.ant-select-dropdown .ant-select-item').filter({ hasText: '普通' }).first();
      const isUrgencyOptionVisible = await urgencyOption.isVisible({ timeout: 5000 }).catch(() => false);
      if (isUrgencyOptionVisible) {
    ...
    // 选择任务状态（必填）- 通过placeholder定位
    const statusSelectInput = page.locator('input[placeholder*="任务状态"]').first();
    ...
    if (isStatusVisible) {
      ...
      // 等待新的下拉菜单出现
      const statusOption = page.locator('.ant-select-dropdown .ant-select-item').filter({ hasText: '待办' }).first();
    ...
    if (errorCount > 0) {
      ...
      // 如果有关键字段错误，尝试修复
      const hasNameError = errors.some(e => e.includes('任务名称'));
      const hasAssigneeError = errors.some(e => e.includes('分配'));
      if (hasNameError) {
        // 重新填写任务名称 - 使用Modal内的输入框
        const nameInput = page.locator('.ant-modal input[placeholder*="任务名称"]:not([placeholder*="搜索"])').first();
        await nameInput.click({ force: true });
        await nameInput.clear();
        const newTaskName = `测试任务_${Date.now()}`;
        await nameInput.fill(newTaskName);
        await nameInput.press('Tab');
        await page.waitForTimeout(500);
        // 再次验证
        const nameValue = await nameInput.inputValue();
      ...
      if (hasAssigneeError) {
        // 重新选择分配人
        await page.keyboard.press('Escape'); // 先关闭可能打开的下拉菜单
        await page.waitForTimeout(300);
        const assigneeSelect = page.locator('.ant-modal .ant-select').first();
        await assigneeSelect.click({ force: true });
    ...
    try {
      ...
      if (stillVisible) {
        ...
        // 尝试截图以便调试
        await page.screenshot({ path: 'test-results/task-modal-error.png' });
    ...
    if (modalClosed) {
      ...
      // 验证任务已创建 - 在表格中查找，使用更灵活的选择器
      const taskCell = page.locator(`td:has-text("${taskName}")`).first();
      const taskInTable = page.locator(`.ant-table-tbody:has-text("${taskName}")`).first();
      
      // 等待任务出现在表格中
      const taskExists = await Promise.race([
        taskCell.waitFor({ state: 'visible', timeout: 10000 }).then(() => true).catch(() => false),
        taskInTable.waitFor({ state: 'visible', timeout: 10000 }).then(() => true).catch(() => false),
      ]);
